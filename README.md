...
- 미국·한국 금융데이터의 거래일과 시장 시간을 예측 시점 기준으로 정렬하고, 결측 처리와 모델링 데이터셋 구축을 담당했습니다.
- 미래 정보가 유입되지 않도록 국내시장 lag 변수와 `shift-before-rolling` 특성을 설계했습니다.
- XGBoost 모델의 학습·튜닝과 시간순 검증을 수행했습니다.
- TreeSHAP 전역·국소 분석으로 주요 변수의 영향 방향과 극단 변동일의 예측 근거를 해석했습니다.
...
Y1에서는 전일 KOSPI 수익률, Y2에서는 최근 변동성 수준이 주요 설명 변수로 나타났습니다. TreeSHAP을 통해 평균적인 변수 중요도뿐 아니라 극단적 변동일의 개별 예측 근거도 사례 단위로 확인했습니다.
